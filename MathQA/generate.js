...
// Read the JSON file
const jsonFilePath = path.join(__dirname, 'challenge_test_assessed.json');
let jsonData;

try {
  const rawData = fs.readFileSync(jsonFilePath, 'utf8');
  jsonData = JSON.parse(rawData);
  console.log(`Successfully loaded ${jsonData.length} questions from challenge_test_assessed.json`);
} catch (error) {
...
<body>
  ...
  <footer>
    <p>&copy; 2025 Math Problems Collection</p>
  </footer>
...
// Write the HTML file
const outputPath = path.join(__dirname, 'math_problems.html');
